...
class StationLocationOptimizerTest {
    ...
    fun `find next to vaporize`() {
        val optimizer = StationLocationOptimizer("testmap3.txt")
        assertThat(optimizer.findNextAsteroidsToVaporize(optimizer.asteroidMap.toMutableSet(), Asteroid(2, 2)))
                .isEqualTo(listOf(Asteroid(4, 0), Asteroid(3, 3), Asteroid(2, 2), Asteroid(1, 1)))
    }
    ...
    fun vaporizeField() {
        ...
        val optimizer = StationLocationOptimizer("testmap3.txt")
        assertThat(optimizer.vaporizeAsteroids()).isEqualTo(listOf(Asteroid(3, 3), Asteroid(2, 2), Asteroid(1, 1), Asteroid(0, 0)))

        val optimizer2 = StationLocationOptimizer("largeMap.txt")
        val list = optimizer2.vaporizeAsteroids()
        val result = list[199].x * 100 + list[199].y
